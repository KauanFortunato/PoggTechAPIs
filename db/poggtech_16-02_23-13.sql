...
--
-- Host: 127.0.0.1
-- Generation Time: Feb 17, 2025 at 12:13 AM
...
(54, 15, 3, 0, '2025-02-14 15:21:24'),
(56, 15, 23, 1, '2025-02-15 02:52:47'),
(57, 15, 3, 1, '2025-02-15 23:49:42'),
(58, 15, 14, 1, '2025-02-15 23:49:55'),
(59, 15, 5, 1, '2025-02-16 00:05:32');
...
INSERT INTO `messages` (`id_message`, `sender_id`, `receiver_id`, `product_id`, `message`, `timestamp`) VALUES
(1, 15, 18, 4, 'Vai pegar a Margarida AKA Clara quando ?', '2025-02-14 20:16:30'),
(2, 18, 15, 4, 'Quando vc me der uma sugadinha...', '2025-02-14 20:17:29'),
(3, 18, 15, 4, 'Seu boboca', '2025-02-15 21:24:37'),
(4, 15, 18, 4, 'GOGOGOOGOGOGOGOGO... Pronto agora já podes', '2025-02-15 21:30:31'),
(5, 15, 19, 5, 'Ainda tás com a Núria???', '2025-02-16 00:22:26'),
(6, 19, 15, 5, 'Não gajo tás maluco, eu larguei ela...', '2025-02-16 00:22:48'),
(7, 19, 18, 23, 'E esse spider-man grosso ai emm?', '2025-02-16 00:24:20'),
(8, 18, 19, 23, 'Queres foto?', '2025-02-16 00:24:31'),
(9, 19, 18, 23, 'Sim pfvr :)', '2025-02-16 00:24:48'),
(10, 18, 15, 18, 'E esse switch ai? 10 conto? Pegar ou largar', '2025-02-16 14:32:32'),
(11, 15, 18, 18, 'Sai do fake rodrigo', '2025-02-16 14:33:17'),
(12, 15, 18, 4, 'Responde ai', '2025-02-16 20:48:11'),
(13, 15, 18, 4, 'responda', '2025-02-16 20:48:18'),
(14, 15, 18, 4, 'ai clarinha', '2025-02-16 20:48:24'),
(15, 15, 18, 4, 'tiraaaaa', '2025-02-16 20:48:28'),
(16, 15, 18, 4, 'ohnnnnnnnn', '2025-02-16 20:48:33'),
(17, 15, 18, 4, 'Ggogogoogogogogoogogo', '2025-02-16 20:48:53'),
(18, 15, 18, 4, 'udjdjd', '2025-02-16 22:11:49'),
(19, 15, 18, 4, 'ta funcionando nigagaaaaa', '2025-02-16 22:14:41');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_message_details`  AS SELECT `m`.`id_message` AS `id_message`, `p`.`user_id` AS `owner_id`, `m`.`sender_id` AS `sender_id`, `sender`.`name` AS `sender_name`, `sender`.`last_name` AS `sender_last_name`, `m`.`receiver_id` AS `receiver_id`, `receiver`.`name` AS `receiver_name`, `receiver`.`last_name` AS `receiver_last_name`, `m`.`product_id` AS `product_id`, `p`.`title` AS `product_title`, `m`.`message` AS `message`, `m`.`timestamp` AS `timestamp`, date_format(`m`.`timestamp`,'%H:%i') AS `timestamp_format` FROM (((`messages` `m` join `users` `sender` on(`m`.`sender_id` = `sender`.`user_id`)) join `users` `receiver` on(`m`.`receiver_id` = `receiver`.`user_id`)) join `products` `p` on(`m`.`product_id` = `p`.`product_id`)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_user_chats`  AS SELECT `m1`.`id_message` AS `id_message`, `p`.`user_id` AS `owner_id`, `m1`.`sender_id` AS `user_id`, `m1`.`receiver_id` AS `chat_with`, `u2`.`name` AS `chat_with_name`, `u2`.`last_name` AS `chat_with_last_name`, `m1`.`product_id` AS `product_id`, `p`.`title` AS `product_title`, `p`.`price` AS `product_price`, `m1`.`message` AS `last_message`, `p`.`image_url` AS `image_product`, `m1`.`timestamp` AS `last_message_time`, date_format(`m1`.`timestamp`,'%H:%i') AS `last_message_time_format` FROM ((`messages` `m1` join `products` `p` on(`m1`.`product_id` = `p`.`product_id`)) join `users` `u2` on(`m1`.`receiver_id` = `u2`.`user_id`)) WHERE `m1`.`timestamp` = (select max(`m2`.`timestamp`) from `messages` `m2` where `m2`.`sender_id` = `m1`.`sender_id` AND `m2`.`receiver_id` = `m1`.`receiver_id` AND `m2`.`product_id` = `m1`.`product_id` OR `m2`.`sender_id` = `m1`.`receiver_id` AND `m2`.`receiver_id` = `m1`.`sender_id` AND `m2`.`product_id` = `m1`.`product_id`)union select `m1`.`id_message` AS `id_message`,`p`.`user_id` AS `owner_id`,`m1`.`receiver_id` AS `user_id`,`m1`.`sender_id` AS `chat_with`,`u1`.`name` AS `chat_with_name`,`u1`.`last_name` AS `chat_with_last_name`,`m1`.`product_id` AS `product_id`,`p`.`title` AS `product_title`,`p`.`price` AS `product_price`,`m1`.`message` AS `last_message`,`p`.`image_url` AS `image_product`,`m1`.`timestamp` AS `last_message_time`,date_format(`m1`.`timestamp`,'%H:%i') AS `last_message_time_format` from ((`messages` `m1` join `products` `p` on(`m1`.`product_id` = `p`.`product_id`)) join `users` `u1` on(`m1`.`sender_id` = `u1`.`user_id`)) where `m1`.`timestamp` = (select max(`m2`.`timestamp`) from `messages` `m2` where `m2`.`sender_id` = `m1`.`sender_id` and `m2`.`receiver_id` = `m1`.`receiver_id` and `m2`.`product_id` = `m1`.`product_id` or `m2`.`sender_id` = `m1`.`receiver_id` and `m2`.`receiver_id` = `m1`.`sender_id` and `m2`.`product_id` = `m1`.`product_id`)  ;
...
ALTER TABLE `messages`
  ...
  ADD KEY `messages_ibfk_3` (`product_id`);
...
ALTER TABLE `messages`
  ...
  ADD CONSTRAINT `messages_ibfk_3` FOREIGN KEY (`product_id`) REFERENCES `products` (`product_id`) ON DELETE CASCADE;
...
ALTER TABLE `user_history`
  ...
  ADD CONSTRAINT `user_history_ibfk_3` FOREIGN KEY (`category`) REFERENCES `categories` (`name`) ON DELETE CASCADE;
COMMIT;
